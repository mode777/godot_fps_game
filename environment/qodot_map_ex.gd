...
func init_texture_loader() -> QodotTextureLoader:
	...
	tex_ldr.enable_compression = get_world_prop(&"enable_compression") != "0"
	var mat = default_material as StandardMaterial3D
	var tex_filter = get_world_prop(&"texture_filter")
	print("Texture compression enabled: %s"%tex_ldr.enable_compression)
	mat.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS_ANISOTROPIC if tex_filter == "0" else BaseMaterial3D.TEXTURE_FILTER_NEAREST_WITH_MIPMAPS_ANISOTROPIC
...
func get_worldspawn():
	if worldspawn:
		...
		var q = qodot as Qodot
		for e in q.map_data.entities:
			if e.properties.classname == "worldspawn":
				worldspawn = e
				return e
		return null
		
func get_world_prop(key: String):
	return get_worldspawn().properties[key]
